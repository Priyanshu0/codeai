...
if openai_api_key:
    ...
    code_string2 = """ You ara a data analyst having experince of more than 10 year,\
        Generate the some very basic question based on the {column_descriptions} of the dataframe\
        that an user can analyse only give the questions that can be answer with the column in column_descriptions\
             and using python code """

    code_template2 = ChatPromptTemplate.from_template(code_string2)
    ...
    user_ques = code_template2.format_messages(column_descriptions=column_descriptions)


    # Initialize ChatOpenAI instance
    chat2 = ChatOpenAI(temperature=0.0, model=model, openai_api_key=openai_api_key)
    ques_sugg=st.checkbox("Need Help with Question ? ")
    if ques_sugg:
        with st.spinner('Wait for it...'):
            with st.expander(" Suggested Questions", expanded=False):
                st.markdown(chat2(user_ques).content)
